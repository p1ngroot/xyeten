...
- **TUI (Terminal UI)** - консольный интерфейс с rich
- **Web Interface** - управление через браузер (опционально)
...
### Terminal UI (TUI) - Рекомендуется

Консольный интерфейс с красивым отображением:

```bash
sudo python3 /opt/rpi-multitool/rpi_multitool_tui.py
```

**Управление клавишами:**
- `1-5` - Выбор модуля (1=Home, 2=Scan, 3=Sniffer, 4=Deauth, 5=Beacon, 6=Jammer)
- `SPACE` - Запуск/остановка текущего модуля
- `s` - Запустить WiFi сканирование
- `↑/↓` - Навигация по списку сетей (выбор цели для deauth)
- `q` - Выход

**Особенности TUI:**
- Живое обновление статистики в реальном времени
- Цветной вывод с подсветкой синтаксиса
- Компактное отображение всей информации на одном экране
